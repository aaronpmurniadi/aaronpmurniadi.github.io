    ...
    el.addEventListener(
      ...
      (e) => {
        ...
        e.clipboardData.setData(
          "text/plain",
          sel.toString().replace(RegExp(SOFTHYPHEN, "g"), "")
        );
        e.clipboardData.setData(
          "text/html",
          div.innerHTML.replace(RegExp(SOFTHYPHEN, "g"), "")
        );
      },
      true
    );
    ...
    const settings = createMapWithDefaults(
      new Map([
        ...
        [
          ...
          (() => {
            const list =
              "abbr,acronym,audio,br,button,code,img,input,kbd,label,math,option,pre,samp,script,style,sub,sup,svg,textarea,var,video";
      ...
      switch (key) {
        case "selectors":
          ...
          o.entries(value).forEach(([sel, selSettings]) => {
            const selectorSettings = createMapWithDefaults(
              new Map([
                ["compound", "hyphen"],
                ["hyphen", SOFTHYPHEN],
                ["leftmin", 0],
            ...
            o.entries(selSettings).forEach(([selSetting, setVal]) => {
  ...
  ((H) => {
    ...
    function collectElements(parent = null, selector = null) {
      ...
      const matchingSelectors =
        C.selectors.join(",") + "," + dontHyphenateSelector;
      ...
      function processElements(el, pLang, sel, isChild = false) {
        ...
        if (langDef === "H9Y") {
          ...
          event.fire(
            "error",
            Error(
              `Element with '${eLang}' found, but '${eLang}.hpb' not loaded. Check language tags!`
            )
          );
        }
    ...
    function createOrphanController(sel) {
      ...
      function controlOrphans(
        ...
        return (
          leadingWhiteSpace +
          lastWord.replace(RegExp(selSettings.hyphen, "g"), "") +
          trailingWhiteSpace
        );
      }
      ...
      return (entity, sel = ".hyphenate") => {
        if (typeof entity !== "string") {
          event.fire(
            "error",
            Error(
              "This use of hyphenators is deprecated. See https://mnater.github.io/Hyphenopoly/Hyphenators.html"
            )
          );
        }
    ...
    function prepareLanguagesObj(
      ...
      C.selectors.forEach((sel) => {
        ...
        selSettings.leftminPerLang.set(
          lang,
          Math.max(
            patternLeftmin,
            selSettings.leftmin,
        ...
        selSettings.rightminPerLang.set(
          lang,
          Math.max(
            patternRightmin,
    ...
    function instantiateWasmEngine(heProm, lang) {
      ...
      function registerSubstitutions(alphalen, exp) {
        if (C.substitute.has(lang)) {
          ...
          subst.forEach((substituer, substituted) => {
            ...
            const substitutedUcc =
              substitutedU === substituted ? 0 : substitutedU.charCodeAt(0);
            alphalen = exp.subst(
      ...
      function handleWasm(res) {
        ...
        prepareLanguagesObj(
          lang,
          encloseHyphenateFunction(baseData, exp.hyphenate),
      ...
      heProm.w
        .then((response) => {
          ...
          return Promise.reject(
            Error(
              `File ${lang}.wasm can't be loaded from ${H.paths.patterndir}`
            )
          );
        })
        .then(handleWasm, (e) => {
